 ...
 */
@ThreadSafe
public class OrderBy extends Node implements Query, Serializable {
     ...
     */
    public OrderBy(final String cmd,
                   final String data,
        ...
        if (cmd.isEmpty()) {
            throw new InvalidParameterException(INVALID_ORDERBY_CMD);
        }

        if (data.isEmpty()) {
            throw new InvalidParameterException(INVALID_ORDERBY_DATA);
        }
    }
